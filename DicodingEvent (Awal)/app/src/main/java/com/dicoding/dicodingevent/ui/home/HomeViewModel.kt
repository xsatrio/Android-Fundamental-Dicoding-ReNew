...
class HomeViewModel : ViewModel() {
    ...
    private val _carouselEvents = MutableLiveData<List<ListEventsItem>>()
    val carouselEvents: LiveData<List<ListEventsItem>> = _carouselEvents
    ...
    fun getCarousel(context: Context) {
        _isLoading.value = true
        val client = ApiConfig.getApiService().listEvent("1")
        client.enqueue(object : Callback<EventResponse> {
            override fun onResponse(call: Call<EventResponse>, response: Response<EventResponse>) {
                _isLoading.value = false
                if (response.isSuccessful) {
                    _carouselEvents.value = response.body()?.listEvents?.take(5) ?: listOf()
                } else {
    ...
    fun getEvents(context: Context) {
        ...
        client.enqueue(object : Callback<EventResponse> {
            override fun onResponse(call: Call<EventResponse>, response: Response<EventResponse>) {
                _isLoading.value = false
                if (response.isSuccessful) {
                    _events.value = response.body()?.listEvents?.take(5) ?: listOf()
                } else {
                    _errorMessage.value = when (response.code()) {
                        ResponseCode.RC_UNAUTHORIZED.value -> context.getString(R.string.error401)
                        ResponseCode.RC_FORBIDDEN.value -> context.getString(R.string.error403)
